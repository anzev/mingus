...
class Documize():
    ...
    def generate_module_wikidocs(self):
        ...
        res += "----\n\n= %s =\n\n%s\n\n----\n\n"  % (self.module_string, self.strip_license(self.module.__doc__))
...
def generate_package_wikidocs(package_string, file_prefix = 'ref', file_suffix = ".wiki"):
    d = Documize()
    ...
    for element in dir(package):
        if not callable(element):
            ...
            if type(eval(fullname)) == types.ModuleType or type(eval(fullname)) == type:
